...
class AgentClientAuthorisationService @Inject() (
  ...
  def getAgentRequests(clientType: String, clientIdOpt: ClientIdentifiers)(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[Seq[AgentRequest]] = {

    val storedItsaInvitations = invitations(clientIdOpt.mtdItId)(clientId => acaConnector.getInvitation(MtdItId(clientId.value)))
    val storedIrvInvitations =
      invitations(clientIdOpt.nino)(clientId => acaConnector.getInvitation(relationshipManagementService.removeNinoSpaces(Nino(clientId.value))))
    val storedAltItsaInvitations = invitations(clientIdOpt.nino)(clientId =>
      acaConnector.getInvitation(relationshipManagementService.removeNinoSpaces(Nino(clientId.value)), true)
    )
    ...
    val storedCbcInvitations = invitations(
      clientIdOpt.cbcUkRef // won't have both but maybe one.
        .orElse(clientIdOpt.cbcNonUkRef)
    )(clientId => acaConnector.getInvitation(CbcId(clientId.value)))
    val storedPlrInvitations = invitations(clientIdOpt.plrId)(clientId => acaConnector.getInvitation(PlrId(clientId.value)))

    val relationshipsWithAgencyNamesWithStoredInvitations = for {
      storedInvitations <- Future
                             .sequence(
                               Seq(
                                 ...
                                 storedCbcInvitations,
                                 storedPlrInvitations
                               )
                             )
                             .map(_.flatten)
    ...
    relationshipsWithAgencyNamesWithStoredInvitations.flatMap { case (agencyName, storedInvites, agentRefs) =>
      Future
        .traverse(storedInvites)(si =>
          for {
