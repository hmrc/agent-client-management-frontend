...
case class ClientIdentifiers(mtdItId: Option[MtdItId], nino:Option[Nino], vrn:Option[Vrn], utr: Option[Utr],
  ...
  val haveAtLeastOneFieldDefined: Boolean = mtdItId.isDefined || nino.isDefined || vrn.isDefined || utr.isDefined || cgtRef.isDefined || urn.isDefined || pptRef.isDefined || cbcUkRef.isDefined || cbcNonUkRef.isDefined || plrId.isDefined
  val hasOnlyNino: Boolean = nino.isDefined && mtdItId.isEmpty && vrn.isEmpty && utr.isEmpty && cgtRef.isEmpty && urn.isEmpty && pptRef.isEmpty && cbcUkRef.isEmpty && cbcNonUkRef.isEmpty && plrId.isEmpty
