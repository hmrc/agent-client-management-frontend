...
trait AuthStubs {
  ...
  def authorisedAsClientMtdItId[A](request: FakeRequest[A], mtdItId: String): FakeRequest[A] =
    authenticated(request, Set(Enrolment("HMRC-MTD-IT", "MTDITID", mtdItId)), isAgent = false)
  def authorisedAsClientTrustNtUrn[A](request: FakeRequest[A], urn: String): FakeRequest[A] =
    authenticated(request, Set(Enrolment("HMRC-TERSNT-ORG", "URN", urn)), isAgent = false)
  def authorisedAsClientMtdItIdWithIrSa[A](request: FakeRequest[A], mtdItId: String, utr: String): FakeRequest[A] =
    authenticated(request, Set(Enrolment("HMRC-MTD-IT", "MTDITID", mtdItId), Enrolment("IR-SA", "UTR", utr)), isAgent = false)
  def authorisedAsClientNi[A](request: FakeRequest[A], nino: String): FakeRequest[A] =
    authenticated(request, Set(Enrolment("HMRC-NI", "NINO", nino)), isAgent = false)
  def authorisedAsClientVat[A](request: FakeRequest[A], vrn: String): FakeRequest[A] =
    authenticated(request, Set(Enrolment("HMRC-MTD-VAT", "VRN", vrn)), isAgent = false)
  def authorisedAsClientAll[A](
