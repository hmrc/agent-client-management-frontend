...
class AuditServiceSpec extends UnitSpec with MockitoSugar with Eventually {
  ...
  "AuditEvent" should {
    "send an ClientRemovedAgentServiceAuthorisation Event" in {
      val mockConnector = mock[AuditConnector]
      val auditService = new AuditService(mockConnector)
      ...
      await(auditService.sendClientRemovedAgentServiceAuthorisation(
        "userID", arn, "HMRC-MTD-IT", "WM123456C"
      ...
      eventually {
        ...
        sentEvent.auditSource shouldBe "agent-client-management-frontend"
        sentEvent.detail("userID") shouldBe "userID"
